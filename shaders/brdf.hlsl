...
float4 frag(FS_INPUT input) : SV_Target {
    ...
    for (int i = 0; i < NUM_SAMPLES; ++i) {
        ...
        // xi.x = 0.35;
        // xi.y = 0.5;
        float3 L = sampleNormalFromLongLat(float2(xi.x*2, acos(pow(xi.y, 1.0 / (a+1)))/PI));
        ...
        float lod = 0.5 * log2((float)skySize.x * (float)skySize.y / (float)NUM_SAMPLES) - 0.5 * log2(max(D, 0.00001));
        lod = max(lod, 0);
        lod *= 0.01;
